function recursionExpand(node, lenArray, index = 0) {
...
function recursionDefine(node_main, node_define) {
    let children_length;
    for (let i = 0; i < node_define.length; i) {
        let length = [];
        if (node_define[i].constructor.name === `Array`) {
            let length_i = [];
            for (let j = 0; j < node_define[i].length; j++) {
                length_i.push(node_define[i].length);
            }
...
class Tensor extends Array {
    constructor(tensor = undefined, length = []) {
        super();
        if (!tensor)
